...
description: >-
  Use when starting feature work or executing a plan that must not touch the
  current workspace, or when parallel subagents write to overlapping files —
  ensures an isolated workspace exists via native tools or a git worktree
  fallback.
---
...
**Cross-links:** Cleanup of worktrees (delete branch, remove worktree) lives
in `finishing-work`. Parallel subagents that touch overlapping files must
enter their own worktree — see `dispatching-parallel-agents`.
...
submodules. Before concluding "already in a worktree," verify you are not in
a submodule:
...
linked worktree. Skip to Step 2 (Project Setup). Do NOT create another
worktree.
...
**Only use this if Step 1a does not apply** — you have no native worktree
tool available. Create a worktree manually using git.
...
3. **If there is no other guidance available**, default to `.worktrees/` at
   the project root.
...
- **Fix:** Always use `git check-ignore` before creating project-local
  worktree

### Assuming directory location

- **Problem:** Creates inconsistency, violates project conventions
- **Fix:** Follow priority: explicit instructions > existing project-local
  directory > default
...
- Use `git worktree add` when you have a native worktree tool (e.g.,
  `EnterWorktree`). This is the #1 mistake — if you have it, use it.
